...
namespace Phalanx\Agents\Tests\Integration;
...
use Phalanx\Agents\Turn\Config as TurnConfig;
...
use Phalanx\Stream\Channel as StreamChannel;
...
final class ChannelBackedLoopTest extends PhalanxTestCase
{
    ...
    public function scale_concurrent_activities(): void
    {
        ...
        $this->scope->run(static function (ExecutionScope $scope) use ($activityCount, $cuesPerActivity, $expected): void {
            $collector = new StreamChannel($activityCount);
...
final class InlineEffectHandler
{
     ...
     */
    public function handle(TaskScope $scope, Requested $request, StreamChannel $channel, string $activityId): array
    {
        $at = new \DateTimeImmutable();
...
final class ProofChannelLoop
{
    ...
    {
        $cueChannel = new StreamChannel($this->channelBuffer);
        ...
        $runtime = (new ScopedRuntimeFactory())($scope);
