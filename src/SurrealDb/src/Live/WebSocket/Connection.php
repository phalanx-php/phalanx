...
class Connection implements \Phalanx\SurrealDb\Live\Connection
{
    ...
    public function __construct(
        ...
        private readonly \Phalanx\SurrealDb\Live\Socket $socket,
    ...
    public function request(string $method, array $params = []): mixed
    {
        ...
        try {
            ...
            if ($message === null) {
                ...
                throw new \Phalanx\SurrealDb\Exception("SurrealDb live RPC {$method} timed out.");
            }
    ...
    private function handleEnvelope(mixed $envelope): void
    {
        if (!is_array($envelope) || array_is_list($envelope)) {
            throw new \Phalanx\SurrealDb\Exception('SurrealDb live RPC envelope was not a JSON object.');
        }
        ...
        if (!is_array($result)) {
            throw new \Phalanx\SurrealDb\Exception('SurrealDb live notification result was missing.');
        }

        $notification = \Phalanx\SurrealDb\Live\Notification::fromPayload($result);
        ...
        if ($channel !== null) {
            if (!$channel->tryEmit($notification)) {
                ...
                $channel->error(new \Phalanx\SurrealDb\Exception(sprintf(
    ...
    private function handleReply(array $envelope): void
    {
        $id = $envelope['id'];
        if (!is_int($id)) {
            throw new \Phalanx\SurrealDb\Exception('SurrealDb live RPC response id was invalid.');
        }
        ...
        if (array_key_exists('error', $envelope)) {
            $reply->error(\Phalanx\SurrealDb\Exception::fromErrorEnvelope($envelope['error']));

            return;
        }

        if (!array_key_exists('result', $envelope)) {
            $reply->error(new \Phalanx\SurrealDb\Exception('SurrealDb live RPC response was missing result or error.'));
    ...
    private function fail(Throwable $error): void
    {
        $exception = $error instanceof JsonException
            ? new \Phalanx\SurrealDb\Exception("Failed to decode SurrealDb live message: {$error->getMessage()}", previous: $error)
            : $error;
    ...
    {
        if (!$this->isOpen) {
            throw new \Phalanx\SurrealDb\Exception('SurrealDb live connection is closed.');
        }
    }
}
