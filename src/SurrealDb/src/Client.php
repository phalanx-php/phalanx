...
class Client
{
    ...
    public function version(): string
    {
        ...
        if (!is_string($version)) {
            throw new \Phalanx\SurrealDb\Exception('SurrealDb version returned a non-string response.');
        }
    ...
    public function live(string $table, bool $diff = false): \Phalanx\SurrealDb\Live\Subscription
    {
    ...
    /** @param array<string, mixed> $params */
    public function liveQuery(string $query, array $params = []): \Phalanx\SurrealDb\Live\Subscription
    {
        $merged = [...$this->params, ...$params];
        $result = $merged === []
    ...
    private static function extractLiveQueryId(mixed $result, string $method): string
    {
        ...
        throw new \Phalanx\SurrealDb\Exception("SurrealDb {$method} did not return a live query id.");
    }
    ...
    private static function extractToken(mixed $result, string $method): ?string
    {
        ...
        throw new \Phalanx\SurrealDb\Exception("SurrealDb {$method} returned a non-token response.");
    }

    private function liveConnection(): \Phalanx\SurrealDb\Live\Connection
    {
        $this->guard->assertOpen();
        ...
        if ($this->liveTransport === null) {
            throw new \Phalanx\SurrealDb\Exception('SurrealDb live queries require a live transport.');
        }

        if (!$this->scope instanceof ExecutionScope) {
            throw new \Phalanx\SurrealDb\Exception('SurrealDb live queries require an execution scope.');
        }
    ...
    private function subscription(string $queryId): \Phalanx\SurrealDb\Live\Subscription
    {
        $channel = new Channel();
        $connection = $this->liveConnection();
        $connection->subscribe($queryId, $channel);

        return new \Phalanx\SurrealDb\Live\Subscription($queryId, $connection, $channel);
    }

    private function assertNoLiveConnection(string $method): void
    {
        if ($this->liveConnection?->isOpen === true) {
            throw new \Phalanx\SurrealDb\Exception("Cannot call {$method} while SurrealDb live subscriptions are open.");
        }
    }
    ...
    private function defaultCredentials(): array
    {
        if ($this->config->username === null || $this->config->password === null) {
            throw new \Phalanx\SurrealDb\Exception('SurrealDb credentials are not configured.');
        }
