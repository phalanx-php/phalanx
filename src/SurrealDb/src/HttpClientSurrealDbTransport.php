...
final class HttpClientSurrealDbTransport implements SurrealDbTransport
{
    ...
    public function rpc(
        ...
        if (is_array($data) && array_key_exists('error', $data)) {
            throw SurrealDbException::fromErrorEnvelope($data['error']);
        }
    ...
    private static function encodeJson(array $payload): string
    {
        try {
            ...
            throw new SurrealDbException("Failed to encode SurrealDb RPC payload: {$e->getMessage()}", previous: $e);
        }
    }

    /** @return array<string, list<string>> */
    private function headers(SurrealDbConfig $config, ?string $token): array
    {
    ...
    private function head(Scope&Suspendable $scope, SurrealDbConfig $config, ?string $token, string $path): int
    {
        $request = new HttpRequest(
            method: 'GET',
    ...
    private function decodeResponse(HttpResponse $response, int $expectedId): mixed
    {
        if (!$response->successful) {
            throw new SurrealDbException("SurrealDb HTTP request failed with status {$response->status}.");
        }
        ...
        try {
            ...
            throw new SurrealDbException("Failed to decode SurrealDb response: {$e->getMessage()}", previous: $e);
        }

        if (!is_array($data) || array_is_list($data)) {
            throw new SurrealDbException('SurrealDb RPC response was not a JSON object.');
        }

        if (array_key_exists('id', $data) && (int) $data['id'] !== $expectedId) {
            throw new SurrealDbException("SurrealDb RPC response id mismatch: expected {$expectedId}.");
        }

        if (!array_key_exists('result', $data) && !array_key_exists('error', $data)) {
            throw new SurrealDbException('SurrealDb RPC response was missing result or error.');
        }
