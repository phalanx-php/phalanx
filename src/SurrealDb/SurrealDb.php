...
class SurrealDb
{
    ...
    private ?SurrealDbLiveConnection $liveConnection = null;

    public function __construct(
        private SurrealDbConfig $config,
        private readonly SurrealDbTransport $transport,
        private readonly TaskScope $scope,
        private readonly SurrealDbScopeGuard $guard = new SurrealDbScopeGuard(),
        private readonly ?SurrealDbLiveTransport $liveTransport = null,
    ) {
    ...
    public function version(): string
    {
        ...
        if (!is_string($version)) {
            throw new SurrealDbException('SurrealDb version returned a non-string response.');
        }
    ...
    public function live(string $table, bool $diff = false): SurrealDbLiveSubscription
    {
    ...
    /** @param array<string, mixed> $params */
    public function liveQuery(string $query, array $params = []): SurrealDbLiveSubscription
    {
        $merged = [...$this->params, ...$params];
        $result = $merged === []
    ...
    private static function extractLiveQueryId(mixed $result, string $method): string
    {
        ...
        throw new SurrealDbException("SurrealDb {$method} did not return a live query id.");
    }
    ...
    private static function extractToken(mixed $result, string $method): ?string
    {
        ...
        throw new SurrealDbException("SurrealDb {$method} returned a non-token response.");
    }

    private function liveConnection(): SurrealDbLiveConnection
    {
        $this->guard->assertOpen();
        ...
        if ($this->liveTransport === null) {
            throw new SurrealDbException('SurrealDb live queries require a live transport.');
        }

        if (!$this->scope instanceof ExecutionScope) {
            throw new SurrealDbException('SurrealDb live queries require an execution scope.');
        }
    ...
    private function subscription(string $queryId): SurrealDbLiveSubscription
    {
        $channel = new Channel();
        $connection = $this->liveConnection();
        $connection->subscribe($queryId, $channel);

        return new SurrealDbLiveSubscription($queryId, $connection, $channel);
    }

    private function assertNoLiveConnection(string $method): void
    {
        if ($this->liveConnection?->isOpen === true) {
            throw new SurrealDbException("Cannot call {$method} while SurrealDb live subscriptions are open.");
        }
    }
    ...
    private function defaultCredentials(): array
    {
        if ($this->config->username === null || $this->config->password === null) {
            throw new SurrealDbException('SurrealDb credentials are not configured.');
        }
