...
final class ClientLiveTest extends PhalanxTestCase
{
    #[Test]
    ...
    public function subscriptionReceivesNotificationsAndKillsQuery(): void
    {
        ...
        $result = $this->scope->run(
            static function (ExecutionScope $scope) use ($connection, $transport): array {
                ...
                $connection->emit('olympus-live', new \Phalanx\SurrealDb\Live\Notification(
        ...
        self::assertSame(\Phalanx\SurrealDb\Live\Action::Create, $result['action']);
...
final class FakeLiveTransport implements \Phalanx\SurrealDb\Live\Transport
{
...
final class FakeLiveConnection implements \Phalanx\SurrealDb\Live\Connection
{
    public bool $isOpen {
        get => $this->open;
    }
    ...
    public function emit(string $queryId, \Phalanx\SurrealDb\Live\Notification $notification): void
    {
