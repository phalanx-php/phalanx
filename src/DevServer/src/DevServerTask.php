...
/**
 * Runtime driver dispatched by DevServerApplicationBuilder onto the Runtime
 ...
 * the Runtime scope dispose (after this method returns) re-runs the
 ...
 * coroutine-mode HTTP server and DevServer refuses to embed raw Swoole
 * HTTP server constructs without Runtime lifecycle wrapping. The feature
 * returns when a Http coroutine runner lands. File watchers still
 * trigger process restarts in the meantime.
 */
final class DevServerTask implements Executable
{
    ...
    public function __invoke(ExecutionScope $scope): int
    {
        ...
        try {
            ...
            $multiplexer->writeLine("\033[31m[dev-server] Readiness failed: {$e->getMessage()}\033[0m");
            self::shutdownProcesses($managed, $multiplexer, $this->quiet);
        ...
        if (!$this->quiet) {
            ...
            $multiplexer->writeLine("\033[1;32m[dev-server] All processes ready.\033[0m");
            $multiplexer->writeLine('');
        }
        ...
        if (!$this->quiet) {
            $multiplexer->writeLine('');
            $multiplexer->writeLine("\033[33m[dev-server] Shutting down...\033[0m");
        }

        self::shutdownProcesses($managed, $multiplexer, $this->quiet);

        if (!$this->quiet) {
            $multiplexer->writeLine("\033[33m[dev-server] All processes stopped.\033[0m");
        }
    ...
    {
        $multiplexer->writeLine("\033[1m[dev-server] Starting processes:\033[0m");
    ...
    private static function wireServerCrashWatchdog(
        ...
        foreach ($managed as $mp) {
            ...
            $mp->onCrash(static function () use ($name, $multiplexer, $scope): void {
                $multiplexer->writeLine(
                    "\033[31m[dev-server] Server process '{$name}' crashed. Shutting down.\033[0m"
                );
                if (!$scope->isCancelled) {
                    $scope->cancellation()->cancel();
                }
    ...
    private static function shutdownProcesses(array $managed, Multiplexer $multiplexer, bool $quiet): void
    {
        foreach ($managed as $mp) {
            $mp->stop();
            if (!$quiet) {
                $multiplexer->writeLine("\033[2m[dev-server] {$mp->config->name} stopped.\033[0m");
            }
        }
    }
    ...
    private static function makeWatchCallback(
        ...
        return static function (array $changed) use ($mp, $name, $multiplexer, $scope): void {
            ...
            $multiplexer->writeLine(
                "\033[33m[dev-server] Change detected ({$label}). Restarting {$name}...\033[0m"
            );

            try {
                $mp->restart($scope, $multiplexer);
                $multiplexer->writeLine("\033[32m[dev-server] {$name} restarted.\033[0m");
            } catch (Cancelled $e) {
                throw $e;
            } catch (Throwable $e) {
                $multiplexer->writeLine("\033[31m[dev-server] Restart failed for {$name}: {$e->getMessage()}\033[0m");
            }
        };
    }
    ...
    private function startWatchers(
        ...
        foreach ($managed as $mp) {
            ...
            if (!$this->quiet) {
                ...
                $multiplexer->writeLine(
                    "\033[2m[dev-server] Watching {$name}: " . implode(', ', $mp->config->watchPaths) . " [{$exts}]\033[0m"
                );
            }
