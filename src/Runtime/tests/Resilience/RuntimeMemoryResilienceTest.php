...
class RuntimeMemoryResilienceTest extends PhalanxTestCase
{
    ...
    public function testReleaseRemovesResourceEdgesLeasesAndAnnotations(): void
    {
        ...
        try {
            $parent = $memory->resources->open(RuntimeResourceSid::Test, id: 'parent');
            $child = $memory->resources->open(RuntimeResourceSid::Test, id: 'child', parentResourceId: 'parent');
    ...
    public function testTerminalTransitionRaceKeepsFirstTerminalTruth(): void
    {
        ...
        try {
            $opened = $memory->resources->open(RuntimeResourceSid::Test, id: 'race-resource');
            ...
            $lateTransitions = array_filter(
                ...
                static fn($event): bool => $event->type === RuntimeEventSid::ResourceLateTransition->value,
            );
    ...
    public function testResourceCapacityFailureIsExplicit(): void
    {
        ...
        try {
            for ($i = 0; $i < 1024; $i++) {
                $memory->resources->open(RuntimeResourceSid::Test, id: "resource-{$i}");
            }
    ...
    public function testAnnotationCapacityFailureIsExplicit(): void
    {
        ...
        try {
            $handle = $memory->resources->open(RuntimeResourceSid::Test, id: 'one');
            for ($i = 0; $i < 1024; $i++) {
    ...
    public function testEventOverflowIsDiagnosticAndDoesNotChangeCurrentTruth(): void
    {
        ...
        try {
            ...
            for ($i = 0; $i < 8; $i++) {
                $memory->resources->recordEvent($active, RuntimeEventSid::RunRunning);
            }

            self::assertSame(ManagedResourceState::Active, $memory->resources->get('one')?->state);
            self::assertGreaterThan(0, $memory->counters->get(RuntimeCounterSid::RuntimeEventsDropped));
