...
final class QueryScopeTest extends TestCase
{
    ...
    public function testQueriesManagedResourcesWithoutOwningLifecycle(): void
    {
        ...
        try {
            ...
            $parent = $memory->resources->open(RuntimeResourceSid::Test, id: 'parent', ownerScopeId: 'scope-1');
            $active = $memory->resources->activate($parent);
            $child = $memory->resources->open(
                RuntimeResourceSid::TaskRun,
                id: 'child',
                parentResourceId: 'parent',
                ownerScopeId: 'scope-1',
                ownerRunId: 'run-1',
            );
            $edgeChild = $memory->resources->open(RuntimeResourceSid::TaskRun, id: 'edge-child');
            ...
            self::assertSame(0, $query->liveCount(RuntimeResourceSid::TaskRun));
            self::assertSame(['parent'], self::ids($query->live()));
            self::assertSame(['child', 'edge-child', 'parent'], self::ids($query->all()));
            self::assertSame(['child', 'edge-child'], self::ids($query->all(RuntimeResourceSid::TaskRun)));
            self::assertSame(['child', 'parent'], self::ids($query->byOwnerScope('scope-1')));
            self::assertSame(['child'], self::ids($query->byOwnerScope('scope-1', RuntimeResourceSid::TaskRun)));
            self::assertSame(['child'], self::ids($query->byOwnerRun('run-1')));
            self::assertSame(['child'], self::ids($query->byOwnerRun('run-1', RuntimeResourceSid::TaskRun)));
            self::assertSame(['child', 'edge-child'], self::ids($query->childrenOf('parent')));
            self::assertSame(
                ['child', 'edge-child'],
                self::ids($query->childrenOf('parent', RuntimeResourceSid::TaskRun)),
            );
            self::assertSame([], self::ids($query->childrenOf('parent', RuntimeResourceSid::Test)));
            self::assertSame(['http.label' => 'parent-resource'], $query->annotations('parent'));
            ...
            self::assertSame(0, $query->stateCounts(RuntimeResourceSid::TaskRun)[ManagedResourceState::Active->value]);
            self::assertSame(2, $query->stateCounts(RuntimeResourceSid::TaskRun)[ManagedResourceState::Closed->value]);
        } finally {
            $memory->shutdown();
        }
    }
    ...
    public function testQueryScopeProjectsLeases(): void
    {
        ...
        try {
            $query = new QueryScope($memory);
            $resource = $memory->resources->open(RuntimeResourceSid::Test, id: 'leased');
    ...
    public function testQueryScopeDoesNotProjectExpiredAnnotations(): void
    {
        ...
        try {
            $query = new QueryScope($memory);
            $resource = $memory->resources->open(RuntimeResourceSid::Test, id: 'annotated');
            ...
            self::assertSame(['http.label' => 'current'], $query->annotations('annotated'));
        } finally {
            $memory->shutdown();
        }
    }
...
{
    case Label = 'http.label';
    case Temporary = 'http.temporary';
