...
namespace Phalanx\Runtime\Tests\Unit\Runtime\Memory;
 ...
 * operation Http uses when an HTTP connection escalates to SSE/WS mid-
...
final class ManagedResourceUpgradeTest extends TestCase
{
    ...
    {
        $handle = $this->memory->resources->open(RuntimeResourceSid::Test, id: 'sse-1');
        $active = $this->memory->resources->activate($handle);
        $originalGeneration = $active->generation;

        $upgraded = $this->memory->resources->upgrade($active, 'http.sse_stream');

        self::assertSame('sse-1', $upgraded->id);
        self::assertSame('http.sse_stream', $upgraded->type);
        self::assertSame($originalGeneration + 1, $upgraded->generation);
    }

    public function testUpgradePreservesResourceLifecycle(): void
    {
        $handle = $this->memory->resources->open(RuntimeResourceSid::Test, id: 'ws-1');
        $active = $this->memory->resources->activate($handle);

        $upgraded = $this->memory->resources->upgrade($active, 'http.ws_session');

        $resource = $this->memory->resources->get('ws-1');
        self::assertNotNull($resource);
        self::assertSame(ManagedResourceState::Active, $resource->state);
        self::assertSame('http.ws_session', $resource->type);
        self::assertSame($upgraded->generation, $resource->generation);
    }

    public function testUpgradeIsIdempotentForSameType(): void
    {
        $handle = $this->memory->resources->open(RuntimeResourceSid::Test, id: 'noop-1');
        $active = $this->memory->resources->activate($handle);

        $reupgraded = $this->memory->resources->upgrade($active, RuntimeResourceSid::Test);
