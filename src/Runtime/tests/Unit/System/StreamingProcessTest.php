...
namespace Phalanx\Runtime\Tests\Unit\System;
 ...
 * The public surface is an Runtime-owned process resource plus a streaming handle.
 */
final class StreamingProcessTest extends PhalanxTestCase
{
    ...
    public function testResourceIsReleasedOnClose(): void
    {
        $result = $this->scope->run(static function (ExecutionScope $scope): int {
            ...
            $liveCount = $scope->runtime->memory->resources->liveCount(RuntimeResourceSid::StreamingProcess);
    ...
    public function testWritesToStdinReadsStdoutAndWaitsForExit(): void
    {
        $result = $this->scope->run(static function (ExecutionScope $scope): array {
            ...
            return [$written, $line, $exitCode, $scope->runtime->memory->resources->liveCount(RuntimeResourceSid::StreamingProcess)];
        });
    ...
    public function testReadLineTimeoutReturnsEmptyWithoutBlockingScheduler(): void
    {
        $result = $this->scope->run(static function (ExecutionScope $scope): array {
            ...
            return [$line, $schedulerAdvanced, $scope->runtime->memory->resources->liveCount(RuntimeResourceSid::StreamingProcess)];
        });
    ...
    public function testWaitWithoutTimeoutDoesNotBlockScheduler(): void
    {
        $result = $this->scope->run(static function (ExecutionScope $scope): array {
            ...
            return [$exitCode, $schedulerAdvanced, $scope->runtime->memory->resources->liveCount(RuntimeResourceSid::StreamingProcess)];
        });
    ...
    public function testWaitTimeoutStopAndCloseAreCleanupSafe(): void
    {
        $result = $this->scope->run(static function (ExecutionScope $scope): array {
            ...
            return [$wait, $state, $scope->runtime->memory->resources->liveCount(RuntimeResourceSid::StreamingProcess)];
        });
    ...
    public function testKillAndCloseAreIdempotent(): void
    {
        $result = $this->scope->run(static function (ExecutionScope $scope): array {
            ...
            return [$state, $scope->runtime->memory->resources->liveCount(RuntimeResourceSid::StreamingProcess)];
        });
