...
final class PhalanxTestCaseTest extends PhalanxTestCase
{
    ...
    {
        $handle = $this->scope->memory->resources->open(RuntimeResourceSid::Test, id: 'leaky-test-resource');
        $active = $this->scope->memory->resources->activate($handle);

        self::assertSame(1, $this->scope->memory->resources->liveCount(RuntimeResourceSid::Test));
    ...
    {
        $handle = $this->scope->memory->resources->open(RuntimeResourceSid::Test, id: 'typed-handle');
        $active = $this->scope->memory->resources->activate($handle);

        try {
            $this->scope->expect->handles()->closed(RuntimeResourceSid::Test);
            self::fail('Expected typed handle assertion to see the live resource.');
        } catch (AssertionFailedError $e) {
            self::assertStringContainsString('runtime.test handle', $e->getMessage());
        } finally {
            $this->scope->memory->resources->close($active, 'test_cleanup');
