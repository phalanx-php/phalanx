...
class StreamingProcess
{
    ...
    public function start(TaskScope&TaskExecutor $scope, bool $closeOnScopeDispose = true): StreamingProcessHandle
    {
        ...
        try {
            ...
            $ownerRunId = method_exists($scope, 'currentRunId') ? $scope->currentRunId() : null;
            $resource = $scope->runtime->memory->resources->open(
                RuntimeResourceSid::StreamingProcess,
            ...
            $scope->runtime->memory->resources->annotate(
                $resource,
                RuntimeAnnotationSid::ProcessCommand,
                $this->commandHead(),
            );
            $scope->runtime->memory->resources->annotate($resource, RuntimeAnnotationSid::ProcessPid, $pid);
            $scope->runtime->memory->resources->annotate(
                $resource,
                RuntimeAnnotationSid::ProcessState,
                StreamingProcessState::Running->value,
            );
            $scope->runtime->memory->resources->annotate($resource, RuntimeAnnotationSid::ProcessCwd, $this->cwd ?? '');
            $active = $scope->runtime->memory->resources->activate($resource);
            $scope->runtime->memory->resources->recordEvent($active, RuntimeEventSid::ProcessStarted, (string) $pid);
