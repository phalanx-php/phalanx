...
/**
 * Swoole reactor backend selection.
 *
 * The reactor type is a server-instance setting (`$server->set(['reactor_type' => ...])`)
 * not a coroutine-global option. This enum is the typed representation Runtime
 * exposes for Http/WebSocket/etc. server bundles to read from RuntimePolicy and
 * apply to their server config.
 *
 * Auto leaves selection to Swoole's autodetect (kqueue on macOS, epoll on
 * modern Linux). IoUring requires Linux 5.13+ and Swoole built with
...
enum ReactorType: string
{
    ...
    /**
     * Returns the constant value Swoole's `Server::set` expects, or
     * null when Auto (caller should omit the key entirely so Swoole's
    ...
    /**
     * io_uring landed in Linux 5.1 but the polished syscall surface Swoole
