...
final readonly class ManagedResourceRegistry
{
    ...
    public function upgrade(
        ...
        try {
            ...
            $event = $this->events->record(
                RuntimeEventSid::ResourceUpgraded,
                resourceId: $id,
    ...
    public function addLease(string $ownerResourceId, string $ownerRunId, array $lease): void
    {
        ...
        $this->events->record(
            RuntimeEventSid::ResourceLeaseAcquired,
    ...
    public function releaseLease(
        ...
        foreach ($this->tables->resourceLeases as $leaseId => $row) {
            ...
            $this->events->record(
                RuntimeEventSid::ResourceLeaseReleased,
                resourceId: $ownerResourceId,
                valueA: $domain,
                valueB: $resourceKey,
            );
    ...
    public function release(string $id): void
    {
        ...
        try {
            ...
            foreach ($edgeIds as $edgeId) {
                $this->tables->resourceEdges->del($edgeId);
            }
            ...
            foreach ($leaseIds as $leaseId) {
                $this->tables->resourceLeases->del($leaseId);
            }

            $annotationIds = [];
            foreach ($this->tables->resourceAnnotations as $annotationId => $row) {
                if (is_array($row) && (string) $row['resource_id'] === $id) {
                    $annotationIds[] = (string) $annotationId;
                }
            }

            foreach ($annotationIds as $annotationId) {
                $this->tables->resourceAnnotations->del($annotationId);
            }

            $event = $this->events->record(
                RuntimeEventSid::ResourceReleased,
                resourceId: $id,
                dispatchListeners: false,
            );
        } finally {
