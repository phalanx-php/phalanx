...
namespace Phalanx\Tui\Runtime\Lifecycle;
...
final class Loop
{
    ...
    public function __construct(
        ...
        if ($this->maxReviewPasses < 1) {
            throw new \InvalidArgumentException('TUI runtime loop max review passes must be >= 1.');
        }
        ...
        while (true) {
            ...
            if ($verdict->needsRevision()) {
                ...
                if ($reviewPasses > $this->maxReviewPasses) {
                    throw new \LogicException('TUI runtime loop exceeded the maximum review passes.');
                }
    ...
    private function executeOn(WorkContext $ctx, WorkPlanItem $item): void
    {
        ...
        $participant = $this->selectAgentParticipant($ctx, $item);
        ...
        try {
            $result = $participant($ctx, $running);
    ...
    private function selectAgentParticipant(WorkContext $ctx, WorkPlanItem $item): AgentParticipant
    {
        foreach ($this->participants as $participant) {
            if ($participant->supports($ctx, $item)) {
                return $participant;
            }
        }

        if (!$this->primary->supports($ctx, $item)) {
