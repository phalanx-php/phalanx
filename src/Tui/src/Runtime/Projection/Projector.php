...
namespace Phalanx\Tui\Runtime\Projection;
...
final class Projector
{
        ...
        $store->transaction(static function () use ($projector, $event, $store): void {
            match ($event->kind) {
                ...
                EventKind::EffectDenied => throw new \InvalidArgumentException(sprintf(
                    'Runtime event "%s" is not supported by the alpha projector.',
                    $event->kind->value,
                )),
            };
    ...
    private function projectReceived(Event $event, Store $store): void
    {
    ...
    private function projectPrepared(Event $event, Store $store): void
    {
        if ($event->workItem === null) {
            return;
        }
    ...
    private function projectStarted(Event $event, Store $store): void
    {
        $workItem = $this->requireWorkItem($event);
    ...
    private function projectResult(Event $event, Store $store): void
    {
    ...
    private function projectReview(Event $event, Store $store): void
    {
    ...
    private function projectStage(Event $event, Store $store): void
    {
    ...
    private function loopStage(Event $event): LoopStage
    {
    ...
    private function projectMetadata(Event $event, Store $store): void
    {
     ...
     */
    private function projectRuntimeMetadata(array $context, Store $store): void
    {
        if (
     ...
     * @param array<string, mixed> $context
     */
    private function projectContextMetadata(array $context, Store $store): void
    {
        if (
     ...
     * @param array<string, mixed> $context
     */
    private function projectParticipantMetadata(array $context, Store $store): void
    {
    ...
    private function validateReceived(Event $event): void
    {
        if ($event->envelope === null && $event->workItem === null) {
            return;
        }
    ...
    private function validateResult(Event $event): void
    {
        $workItem = $this->requireWorkItem($event);
        $workResult = $this->requireWorkResult($event);
    ...
    private function validateReview(Event $event): void
    {
        $this->requireReviewVerdict($event);
    }

    private function requireEnvelope(Event $event): void
    {
        if ($event->envelope === null) {
            throw new \InvalidArgumentException(sprintf(
                'Runtime event "%s" requires an envelope for projection.',
                $event->kind->value,
            ));
        }
    }
    ...
    private function requireWorkItem(Event $event): WorkItem
    {
        return $event->workItem ?? throw new \InvalidArgumentException(sprintf(
            'Runtime event "%s" requires a work item for projection.',
            $event->kind->value,
        ));
    }
    ...
    private function requireWorkResult(Event $event): WorkResult
    {
        return $event->workResult ?? throw new \InvalidArgumentException(sprintf(
            'Runtime event "%s" requires a work result for projection.',
            $event->kind->value,
        ));
    }
    ...
    private function requireReviewVerdict(Event $event): ReviewVerdict
    {
        return $event->reviewVerdict ?? throw new \InvalidArgumentException(
            'Runtime event "work_reviewed" requires a review verdict for projection.',
        );
    }

    private function requireLoopStage(Event $event): LoopStage
    {
        $stage = $event->context['loop_stage'] ?? null;
        if (!is_string($stage)) {
            throw new \InvalidArgumentException('Runtime event "loop_advanced" requires a loop_stage context string.');
        }

        return LoopStage::tryFrom($stage) ?? throw new \InvalidArgumentException(sprintf(
            'Runtime event "loop_advanced" has unknown loop stage "%s".',
            $stage,
        ));
    }
