...
final class RenderDiagnostics
{
    ...
    private function render(Scope $scope, string $kind, string $target, Closure $render): mixed
    {
        ...
        try {
            ...
            $this->record($scope, TraceType::Lifecycle, 'tui.render.cancelled', $kind, $target, $elapsed);

            throw $e;
        } catch (Throwable $e) {
            $elapsed = ($this->clock)() - $startedAt;
            $this->record($scope, TraceType::Failed, 'tui.render.failed', $kind, $target, $elapsed, $e);
        ...
        if ($elapsed >= $this->slowThresholdSeconds) {
            $this->record($scope, TraceType::Lifecycle, 'tui.render.slow', $kind, $target, $elapsed);
        }
