...
use Phalanx\Agent\Exception\MaxInvocationsReached;
...
final class TurnLoopTest extends TestCase
{
    ...
    public function toolCallPushesToolCallAndToolResultIntoConversationLog(): void
    {
        ...
        $registry = new \Phalanx\Agent\Tool\ToolRegistry();
        $registry->register('lookup_tool', LoopTestTool::class);

        $grantStore = new \Phalanx\Agent\Grant\MemoryGrantStore();
        $scope = new ScopeStub();
        ...
        $dispatcher = new \Phalanx\Agent\Effect\Dispatcher(
            ...
            mcpRegistry: new \Phalanx\Agent\Mcp\McpRegistry(),
        );
...
final class LoopTestTool implements \Phalanx\Agent\Tool\Tool
{
    public function __invoke(TaskScope $scope, \Phalanx\Agent\Effect\Context $ctx): \Phalanx\Agent\Effect\Outcome
    {
        return \Phalanx\Agent\Effect\Outcome::routed(
            \Phalanx\Agent\Effect\Resolution::LocalTool,
            data: 'found',
        );
    }
}
