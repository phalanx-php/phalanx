...
namespace Phalanx\Agent\Mcp\Client;

use Generator;
use Phalanx\Agent\Effect\Outcome;
use Phalanx\Agent\Effect\Resolution;
use Phalanx\Agent\Mcp\JsonRpc\Notification;
use Phalanx\Agent\Mcp\JsonRpc\Request;
use Phalanx\Agent\Mcp\JsonRpc\Response;
...
final class SseConnection implements McpConnection
{
    ...
    public function tools(TaskScope $scope): array
    {
        $initResponse = $this->sendRequest($scope, new Request($this->nextId(), 'initialize', [
            ...
            'capabilities' => [],
            'clientInfo' => ['name' => 'phalanx-agent', 'version' => '0.2'],
        ]));
    ...
    public function invoke(TaskScope $scope, string $toolName, array $args): Outcome
    {
        ...
        if ($response->isError) {
            ...
            return Outcome::failed(
                ...
                AiProvidersOutcome::failed('McpError', $message, $elapsed),
            );
        }
