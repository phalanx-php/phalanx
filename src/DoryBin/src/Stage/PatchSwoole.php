...
final class PatchSwoole implements BuildStage
{
    private(set) string $name = 'patch-swoole';

    private(set) string $description = 'Patch Swoole sources for static build';

    public function __invoke(TaskScope&TaskExecutor $scope, SpcBuildContext $context): StageResult
    {
        $start = hrtime(true);
        $extDir = $context->sourcePath . '/php-src/ext/swoole';
        ...
        if (!$patched) {
            return new StageResult(
                ...
                summary: 'Swoole config.m4 not found at ' . $extDir,
            );
        }
        ...
        return new StageResult(
            ...
            summary: 'Swoole sources patched for static build',
        );
    }
