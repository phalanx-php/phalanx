...
{
    public static function decode(string $payload): \Phalanx\HttpClient\Response
    {
        $boundary = strpos($payload, "\r\n\r\n");
        if ($boundary === false) {
            throw new \Phalanx\HttpClient\Exception('HTTP response missing header/body boundary.');
        }
        ...
        if ($lines === [] || !isset($lines[0])) {
            throw new \Phalanx\HttpClient\Exception('HTTP response missing status line.');
        }
    ...
    private static function parseStatusLine(string $line): array
    {
        if (!preg_match('#^HTTP/(\d\.\d)\s+(\d{3})(?:\s+(.*))?$#', $line, $match)) {
            throw new \Phalanx\HttpClient\Exception("Invalid HTTP status line: {$line}");
        }
    ...
    private static function parseHeaders(array $lines): array
    {
        ...
        foreach ($lines as $line) {
            ...
            $colon = strpos($line, ':');
            if ($colon === false) {
                throw new \Phalanx\HttpClient\Exception("Invalid HTTP header line: {$line}");
            }
    ...
    private static function decodeChunkedBody(string $body): string
    {
        ...
        while ($offset < $length) {
            $eol = strpos($body, "\r\n", $offset);
            if ($eol === false) {
                throw new \Phalanx\HttpClient\Exception('Malformed chunked encoding: missing chunk-size line terminator.');
            }

            $sizeLine = substr($body, $offset, $eol - $offset);
            $sizeHex = strtok($sizeLine, ';');
            if ($sizeHex === false || !preg_match('/^[0-9a-fA-F]+$/', $sizeHex)) {
                throw new \Phalanx\HttpClient\Exception("Malformed chunk size: {$sizeLine}");
            }
            ...
            if ($offset + $size > $length) {
                throw new \Phalanx\HttpClient\Exception('Truncated chunk body.');
            }
            ...
            if (substr($body, $offset, 2) !== "\r\n") {
                throw new \Phalanx\HttpClient\Exception('Missing CRLF after chunk data.');
            }

            $offset += 2;
        }

        throw new \Phalanx\HttpClient\Exception('Chunked body did not end with zero-length chunk.');
    }
}
