...
final class RequestEncoder
{
    ...
    public static function encode(\Phalanx\HttpClient\Request $request, ?string $userAgent = null): array
    {
    ...
    private static function parseUrl(string $url): array
    {
        ...
        if ($parts === false || !isset($parts['host']) || !isset($parts['scheme'])) {
            throw new \Phalanx\HttpClient\Exception("Invalid URL: {$url}");
        }

        $scheme = strtolower($parts['scheme']);
        if ($scheme !== 'http' && $scheme !== 'https') {
            throw new \Phalanx\HttpClient\Exception("Unsupported scheme: {$scheme}");
        }
