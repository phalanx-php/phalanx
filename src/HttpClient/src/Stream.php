...
use Generator;
use Phalanx\HttpClient\Wire\ChunkedDecoder;
 ...
 *
 * HTTP/2 was the original transport but Swoole's
 ...
 * SSE unworkable. Every LLM provider Agent targets accepts HTTP/1.1
...
class Stream
{
    ...
    private function readHeaders(Suspendable $scope): void
    {
        while (($end = strpos($this->buffer, "\r\n\r\n")) === false) {
            if (!$this->fillBuffer($scope)) {
                throw new \Phalanx\HttpClient\Exception(
                    "Stream: connection closed before HTTP headers received ({$this->waitDetail})",
                );
            }
        }
        ...
        if (!preg_match('#^HTTP/(\d\.\d)\s+(\d{3})(?:\s+(.*))?$#', $statusLine, $m)) {
            throw new \Phalanx\HttpClient\Exception("Stream: malformed status line '{$statusLine}'");
        }
        $this->protocol = $m[1];
