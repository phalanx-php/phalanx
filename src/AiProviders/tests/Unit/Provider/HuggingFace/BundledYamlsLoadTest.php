...
use Phalanx\AiProviders\Provider\HuggingFace\InferenceProvider;
 ...
 *
 * huggingface.ai-providers.yaml  → InferenceProvider (custom mapper composition)
 * huggingface-dedicated.ai-providers.yaml → OpenAI\ChatProvider (zero-new-code reuse)
 */
final class BundledYamlsLoadTest extends TestCase
{
    ...
    {
        $config = Loader::fromFile(self::yamlPath('huggingface.ai-providers.yaml'));
    ...
    {
        $config = Loader::fromFile(self::yamlPath('huggingface-dedicated.ai-providers.yaml'));
