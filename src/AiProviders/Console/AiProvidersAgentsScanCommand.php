...
namespace Phalanx\AiProviders\Console;
...
/**
 * Console command that scans a PSR-4 namespace directory for
 * {@see \Phalanx\AiProviders\Agent\Discovered}-annotated classes and writes a
...
final class AiProvidersAgentsScanCommand implements Scopeable
{
     ...
     */
    private static function buildPayload(\Phalanx\AiProviders\Agent\Registry $registry, int $sourceMtime): array
    {
        $fqcns = array_map(
            static fn (\Phalanx\AiProviders\Agent $agent): string => $agent::class,
