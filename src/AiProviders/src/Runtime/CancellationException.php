...
namespace Phalanx\AiProviders\Runtime;
 ...
 * reasons — for example, the Runtime runtime adapter wraps Runtime's `Cancelled` token
 * to preserve the cancellation context across the ai-providers boundary. This is why the
 * class is not `final`: it is an intentional extension point for runtime bridges.
 *
 * AiProviders core code relies only on the type ({@see self::class}) for catch clauses
 * and re-throw guards — it never inspects runtime-specific subclass fields.
 *
 * Thrown by {@see \Phalanx\AiProviders\Runtime\Sync\Runtime::throwIfCancelled()}.
 */
