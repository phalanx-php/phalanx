...
use Phalanx\AiProviders\Conversation\Parser as ParserInterface;
 ...
 *   deduplication via {@see \Phalanx\AiProviders\Series::interleaveByDedup()}.
 *
 * Codex JSONL shape:
...
final class Parser implements ParserInterface
{
     ...
     * @return \Generator<\Phalanx\AiProviders\Conversation\Record>
     */
    ...
    private function parseAll(All $source, StrictMode $mode): Log
    {
        ...
        $tsKey = static fn (\Phalanx\AiProviders\Conversation\Record $r): int =>
            $r->at->getTimestamp();

        // Dedup key: canonical JSON payload hash across all three sources.
        $dedupKey = static fn (\Phalanx\AiProviders\Conversation\Record $r): string =>
