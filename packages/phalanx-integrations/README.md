...
```bash
composer require phalanx/integrations
```
...
use Phalanx\Integrations\Ai\AiServiceBundle;
...
$stream = $claude->stream([
    ClaudeMessage::user('What is Phalanx?'),
    ClaudeMessage::assistant('Phalanx is an async coordination library for PHP 8.4+...'),
...
use Phalanx\Integrations\Twilio\TwilioServiceBundle;
...
Route::create('POST', '/twilio/voice', static function (ExecutionScope $scope): Response {
    $twiml = TwiML::say('Welcome to Phalanx. Press 1 for orders, press 2 for support.')
