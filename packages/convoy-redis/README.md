...
Async Redis with typed commands, pub/sub, and automatic connection management — fully non-blocking, fully integrated with Convoy scopes and services.

## Installation

```bash
composer require convoy/redis
```
...
```php
use Convoy\Redis\RedisServiceBundle;
...
Combine with Convoy's concurrent execution to warm multiple cache keys at once:
