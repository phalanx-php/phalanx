...
namespace Phalanx\Panoply\Tests\Unit\Archon;

use Phalanx\Panoply\Archon\PanoplyAgentsScanCommand;
...
/**
 * Unit tests for {@see PanoplyAgentsScanCommand}.
 *
 ...
 * the command directly without booting an Aegis application.
 *
 * Tests operate against the fixture discovered/ directory to exercise the
 * full scan → cache-write pipeline.
 */
final class PanoplyAgentsScanCommandTest extends TestCase
{
    ...
    {
        $command = new PanoplyAgentsScanCommand(
            self::discoveredDir(),
    ...
    public function cacheJsonContainsExpectedAgentFqcns(): void
    {
        ...
        self::assertContains(
            \Phalanx\Panoply\Tests\Fixtures\Agent\Discovered\PhalanxAgent::class,
            $payload['agents'],
        );
    }
    ...
    {
        $nestedPath = sys_get_temp_dir() . '/panoply_scan_nested_' . uniqid() . '/cache.json';
    ...
    {
        $emptyDir = sys_get_temp_dir() . '/panoply_empty_' . uniqid();
        mkdir($emptyDir, 0755);
    ...
    public function writeCacheThrowsRuntimeExceptionWhenPathIsInvalid(): void
    {
        ...
        $invalidPath = '/dev/null/panoply_test_' . uniqid() . '/cache.json';
    ...
    {
        parent::setUp();
        $this->cacheFile = tempnam(sys_get_temp_dir(), 'panoply_scan_test_') . '.json';
    }
     ...
     * directly without booting an Aegis application.
     */
