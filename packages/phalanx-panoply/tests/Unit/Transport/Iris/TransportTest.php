...
final class TransportTest extends TestCase
{
    ...
    {
        // Slow server: never finishes so the generator can only yield a partial
        // read before we abandon it.
        $serverScript = self::writeServer(
            'header("Content-Type: text/plain"); echo "first chunk"; flush(); sleep(30); echo "never";',
        );
        [$proc, $pipes, $port] = self::startServer($serverScript);
        ...
        try {
            self::bootApp()->scoped(
                static function (ExecutionScope $aegisScope) use ($port, &$chunkSeen): void {
                    $client = Iris::client($aegisScope);
                    $transport = new Transport($client, $aegisScope);
                    $request = Request::of('GET', "http://127.0.0.1:{$port}/");
                    $runtime = new SyncRuntime();
        ...
        // If we reach here without hanging, the stream was released cleanly.
    }
