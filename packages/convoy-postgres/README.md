...
Async PostgreSQL with connection pooling, prepared statements, transactions, and LISTEN/NOTIFY — all non-blocking, all composable with Convoy's concurrency primitives.

## Installation

```bash
composer require convoy/postgres
```
...
```php
use Convoy\Postgres\PgServiceBundle;
...
Because `PgPool` manages multiple connections, you can run queries concurrently through Convoy's concurrency primitives:
...
Combine LISTEN with Convoy's concurrency to fan out event handling across multiple channels without blocking:
