...
use Acme\HttpDemo\Runtime\Support\EventCollectionContains;
use Acme\HttpDemo\Runtime\Support\EventTextExtractor;
use Acme\HttpDemo\Runtime\Support\EventWaiter;
use Acme\HttpDemo\Runtime\Support\FirstEventFinder;
use Acme\HttpDemo\Runtime\Support\HttpStatusWaiter;
use Acme\HttpDemo\Runtime\Support\RawConnectionOpener;
use Acme\HttpDemo\Runtime\Support\SimpleHttpGet;
use Acme\HttpDemo\Runtime\Support\TimelinePrinter;
    ...
    static function (DemoReport $report, AppContext $context): void {
        ...
        try {
            \Swoole\Coroutine\run(static function () use ($host, $port, $report): void {
                ...
                $disconnected           = $waitEvent($host, $port, HttpEventSid::ClientDisconnected->value, 2.0, $disconnectResource);
                ...
                $timeline('client disconnect', [
                    ...
                    ['runtime',  $eventText($host, $port, HttpEventSid::ClientDisconnected->value, 'detected closed client socket', $disconnectResource)],
                ...
                $timeline('managed runtime claims', [
                    ['leases',    $leasesReleased ? 'no http.response leases held idle' : 'leases still held idle'],
