...
/**
 * Streaming provider — Agent v2 activity run.
 *
 * Boots Runtime with AgentBundle, probes Ollama at localhost:11434 and
 * falls back to a deterministic Fake provider when Ollama is unreachable
 * or the requested model is not installed.
 *
 * Runs one Agent activity with a simple wisdom-and-strategy prompt and
 ...
 *
 * Usage:
 *   php demos/agent/01-streaming-providers/demo.php
 *   php -d extension=swoole demos/agent/01-streaming-providers/demo.php
 *   OLLAMA_MODEL=mistral php -d extension=swoole demos/agent/01-streaming-providers/demo.php
 */
...
return static function (array $context): Closure {
    // DemoApp::boot() constructs the Runtime kernel which requires Swoole\Table.
    // Guard before boot so a missing extension produces a clean cannotRun message.
    if (!extension_loaded('swoole')) {
        $inner = DemoReport::demo(
            'Agent Streaming Providers',
            static function (DemoReport $report): void {
                $report->cannotRun(
                    'swoole extension required',
                    'Run with: php -d extension=swoole demos/agent/01-streaming-providers/demo.php',
                );
            },
        );
    ...
    // The probe is a plain curl call with no Runtime dependency; safe to run
    // here before boot.
    $choice = $enabled
        ? DemoProvider::ollamaOrFake($fakeScript, $model, $baseUrl)
        : DemoProvider::fakeOnly($fakeScript);

    $bundle = new AgentBundle(new SingleProviderRouter($choice->provider));
    ...
    $bootClosure = DemoApp::boot(
        ...
        static function (DemoApp $app, DemoReport $report) use ($choice): void {
            ...
            // Run inside the Runtime task so Agent::run() can resolve its services.
            $result = $app->run(
                static function (TaskScope $scope) use ($agent, $config, &$tokenCount, &$fullText): Result {
                    $result = Agent::run($scope, $agent, $config);
            ...
            $report->record(
                'activity completed',
                $result->state === \Phalanx\Agent\Activity\State::Completed,
            );
