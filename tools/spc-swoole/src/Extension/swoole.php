...
// NOTE: config.m4 patch patterns need verification against swoole-src tarball
...
class swoole extends PhpExtensionPackage
{
    ...
    {
        if ($installer->getPhpExtensionPackage('openswoole')) {
            throw new WrongUsageException('swoole and openswoole cannot coexist — they register the same internal classes');
        }
    }

    #[BeforeStage('php', [php::class, 'buildconfForUnix'], 'ext-swoole')]
    #[PatchDescription('Copy Swoole source into php-src (workaround for ArtifactLoader init race)')]
    public function ensureSourceInPlace(): void
    {
        $target = SOURCE_PATH . '/php-src/ext/' . $this->getExtensionName();
        $stash = SOURCE_PATH . '/ext-swoole-stash';

        if (!is_dir($stash)) {
            throw new WrongUsageException(
                "Swoole source stash not found at {$stash}. "
                . 'Download swoole/swoole-src and extract to that path.'
            );
        }
    ...
    public function patchConfigM4ForStaticBuild(): void
    {
        ...
        $content = preg_replace(
            '/swoole_source_file="\$swoole_source_file\s*\\\\\n\s+thirdparty\/nghttp2\/[^"]+"/s',
            '',
            $content,
        );
    ...
    #[PatchDescription('Patch Swoole sources for static PHP build compatibility')]
    public function patchSourcesForStaticBuild(): void
    {
        $extDir = SOURCE_PATH . '/php-src/ext/swoole';

        // Redirect nghttp2 include to buildroot headers
        $httpHeader = "{$extDir}/ext-src/php_swoole_http.h";
        ...
        // Fix curl_share_ce duplicate: make it extern in Swoole's copy
    ...
    public function getUnixConfigureArg(bool $shared, PackageBuilder $builder, PackageInstaller $installer): string
    {
        ...
        if ($installer->getPhpExtensionPackage('swoole-hook-pgsql')) {
            $arg .= ' --with-swoole-pgsql=' . BUILD_ROOT_PATH;
        }

        if ($installer->getPhpExtensionPackage('swoole-hook-mysql')) {
            $arg .= ' --enable-swoole-mysqlnd';
        }
