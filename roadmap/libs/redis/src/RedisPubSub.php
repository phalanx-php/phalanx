...
final class RedisPubSub
{
    ...
    public function subscribe(string ...$channels): Emitter
    {
        ...
        $producer = static function (Channel $ch, ExecutionScope $ctx) use (
            $config,
            $channels,
            $pubsub,
        ): void {
            ...
            try {
                $ctx->call(
                    static fn(): mixed => $client->subscribe($channels, static function (
                        \Redis $redis,
                        string $channel,
                        string $message,
                    ) use ($ch): void {
    ...
    public function removeSubscriber(\Redis $client): void
    {
        $this->subscribers = array_values(
            array_filter($this->subscribers, static fn(\Redis $c): bool => $c !== $client),
        );
    }
    ...
    public function subscribeEach(
        ...
        foreach ($emitter($scope) as $item) {
            $scope->executeFresh(
                Task::of(static function (ExecutionScope $child) use ($handler, $item): mixed {
