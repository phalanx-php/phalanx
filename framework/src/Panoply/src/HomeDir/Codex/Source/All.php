 ...
 * the merge.
 *
 * The `configuredSources` list is determined at construction time from which
...
{
    /** @var list<string> */
    private(set) array $configuredSources;
    ...
    public function __construct(
        ...
        $this->configuredSources = array_keys(array_filter([
    ...
    /**
     * Returns which sources were configured at construction — not a runtime
     * availability probe. Values are one or more of `"sessions"`, `"history"`,
     * `"sqlite"`. A source that appears here but encounters a runtime error
     * (e.g. missing `ext-sqlite3` for `"sqlite"`) will be silently omitted from
     * the merge by {@see Parser::parseAll()}.
     *
     * @return list<string>
     */
